...
## Usage You can use `app.py` to: - Activate the measles detection tool - Modify the threshold - Adapt the model to screen other infectious diseases with skin lesions and associated clinical features ### Adjusting the Threshold To adjust the threshold, edit the following line in `calculate.py` (located in the `utils/md` folder):
